...
export const ProductCard: FC<productCardProps> = ({ product }) => {
  return (
    <figure className="flex flex-col animate-fade-in">
      <ImageLoader src={product.urlImage} alt={product.name} className="rounded-t-md" />
      <figcaption className="bg-background p-4 flex flex-col gap-2 rounded-b-md items-center">
        ...
        <button className="text-white bg-accent py-1.5 rounded-md font-medium hover:bg-accent_hover transition-colors w-full">
          Add to car
        </button>
      </figcaption>
    </figure>
